...
bool Reflection::paintGL()
{
    ...
    // Pass 1b: draw scene reflected onto Y=min.y 
	// Uses default shaders
	g.glClearColor(0.8,0.8,0.8,0);
	g.glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
    ...
    // Pass 1c: draw scene reflected onto Y=min.y 
	// Uses default shaders
	g.glClearColor(0.8,0.8,0.8,0);
	g.glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);

	// Reflection matrix with respect to Y=min.y
	QMatrix4x4 reflection3;
	reflection3.scale(-1,1,1);
	reflection3.translate(-2*scene()->boundingBox().min().x(), 0, 0);

	g.defaultProgram()->setUniformValue("modelViewProjectionMatrix", 	
		camera()->projectionMatrix() * camera()->viewMatrix() * reflection3);
	if (drawPlugin()) drawPlugin()->drawScene();
    ...
    // Pass 3b: Draw mirror (a textured quad) PARET DE LA DRETA
	// Uses our own texture mapping shader
	b = scene()->boundingBox();
    program->bind();
    program->setUniformValue("colorMap", 0);
    ...
    // Pass 3c: Draw mirror (a textured quad) PARET DE LA ESQUERRA
	// Uses our own texture mapping shader
	b = scene()->boundingBox();
    program->bind();
    program->setUniformValue("colorMap", 0);
